...
| 工作负载 | CPU/请求 | 1000 万/月,冷 | 1000 万/月,已预热 | 10 亿/月,冷 | 10 亿/月,已预热 |
| --- | --- | --- | --- | --- | --- |
| 平台原生 `fetch` —— 参照;它用不了代理 | 0.3 ms | $5.00 | $5.00 | $307.40 | $307.40 |
| 连接复用,16 KB 页面 | 1.7 ms | $5.61 | $5.00 | $422.65 | $356.65 |
| 连接复用,1 MB 页面 | 5.6 ms | $6.40 | $5.74 | $501.40 | $435.40 |
| 每请求新建连接,16 KB | 6.5 ms | $6.57 | $5.91 | $518.65 | $452.65 |
| 每请求新建连接,1 MB | 10.4 ms | $7.36 | $6.70 | $597.40 | $531.40 |
| 每请求新建连接,4 MB | 22.4 ms | $9.76 | $9.10 | $837.40 | $771.40 |
| Chrome 身份,连接复用,源站发 `br` 的 1 MB 页面 | 9.8 ms | $7.25 | $6.59 | $586.40 | $520.40 |

CPU 那一列来自**重测后的模型**——开连接 6.4 ms + 每次后续请求 1.6 ms + 每 MB 约 4 ms——而不是六个各自独立引用的数字。Chrome 那一行是同一个模型把 gzip 换成 brotli 解码,正文项因此从约 4 抬到约 8.25 ms/MB;它一次性的 3 ms WASM 实例化是真实的,但摊到一个 isolate 服务的请求数上就消失了。**1.3.0 及以前每一行都高出 40–60%**,因为那背后的模型早于 HTTP/2 连接复用,正文路径也更慢。

"冷"这一列带着实测的全新 isolate 爬坡,摊到每请求 +4.4 ms;"已预热"是同一负载执行 `warmup({ iterations: 5 })` 之后,爬坡降到 +1.1 ms。省下的是每月 $0.66(一千万请求)和 $66(十亿请求),每一行都一样——因为爬坡是 isolate 的属性,不是请求的属性。参照那一行不带爬坡,因为平台自己的 `fetch` 没有 JavaScript 协议栈需要分层编译。
...
| --- | --- | --- | --- | --- |
| 基线 —— gzip、AES-256-GCM、x25519 | 5.60 ms | $435.40 | — | 总是 |
| 源站发 `br` 而不是 gzip | 9.85 ms | $520.40 | **+$85** | 源站选择发 `br` |
| 服务器选中 ChaCha20-Poly1305 | 8.55 ms | $494.40 | **+$59** | 服务器优先选它而非 AES |
| 源站发 `zstd` 而不是 gzip | 8.35 ms | $490.40 | **+$55** | 源站选择发 `zstd` |
| 协商 X25519MLKEM768,每连接 1 个请求 | 10.55 ms | $534.40 | **+$99** | 每次握手 |
| 协商 X25519MLKEM768,每连接 20 个请求 | 5.61 ms | $435.55 | **+$0.15** | 同一次握手,摊薄后 |
...
**在每月一千万请求这个量级上,这些都无关紧要。** 每一行都落在 $5 到 $10 之间,因为包含的配额把它吞掉了——在这个量级上,包含的 CPU 折合每请求 3.0 ms,所以一个连接复用的 16 KB 负载在预热之后完全落在基础费里,其余的也就差几美元。

**到了十亿请求,每一行里有 $297 是请求费**,所有行都一样,而且这个包做什么都改变不了它。剩下能优化的只有 CPU,而在那里,**复用连接与不复用之间在 16 KB 页面上差 $96/月**——这是表里最大的一根杠杆,而它的代价只是让一个 Client 活着。

**在连接复用且已预热的情况下,整套用户态协议栈比平台自己的 `fetch` 贵约 16%** —— $357 对 $307 —— 换来的是平台 `fetch` 根本做不到的事。1.3.0 及以前这个数字是 27%;差距收窄是因为成本模型在 HTTP/2 + 连接复用下**重测过**,不是因为为这张表做了什么优化。
