...
通过代理抓取一个尺寸可控的源站，热态，同一 isolate 上 7 轮以上取中位数，传输走 gzip：
...
区间是真实的，不是不精确：这个平台上绝对 CPU 在不同 isolate、不同轮次之间会差到约 1.5 倍
（同一组测试重跑会落到快慢不同的机器上），所以行内数值取中位数，区间就是同 isolate 重复测量
实际展现的散布。在解码和套接字读取路径改用 BYOB 读之前，每 MB 的项约为 25 ms——旧成本花在
逐块的流边界穿越上，而不是字节本身；见下文。
...
Workers Standard 是每月 $5，含 1000 万请求和 3000 万 CPU 毫秒，超出部分每百万请求 $0.30、每百万
CPU 毫秒 $0.02。把上面的实测代入：

| 工作负载 | 每请求 CPU | 1000 万请求/月 | 10 亿请求/月 | 其中 CPU 费 |
| --- | --- | --- | --- | --- |
| 平台自带 `fetch` —— 仅作参照，它根本没法走代理 | ~1 ms | $5.00 | $321.40 | $19.40 |
| 复用连接，16 KB 页面 | 3.3 ms | $5.06 | $367.40 | $65.40 |
| 复用连接，1 MB 页面 | 9.2 ms | $6.24 | $485.40 | $183.40 |
| 每个请求都开新连接，16 KB | 11 ms | $6.60 | $521.40 | $219.40 |
| 每个请求都开新连接，1 MB | 14.5 ms | $7.30 | $591.40 | $289.40 |
| 每个请求都开新连接，4 MB | 30 ms | $10.40 | $901.40 | $599.40 |

这张表里有三件事值得单独拎出来。

**在每月一千万请求这个量级上，这些都无关紧要。** 每一行都落在 $5 到 $11 之间，因为额度全吃下去了。
在这个量级上，included 的 CPU 折合每请求 3.0 ms，所以任何会复用连接的用法都完全包含在基础费里。

**到十亿请求时，每一行里有 $297 是请求费**，它对所有行都一样，而且这个包做什么都改变不了它。剩下能优化的
只有 CPU 那一列，而在那里，复用连接与否在 16 KB 页面上差 $154/月。

**复用连接时，整套用户态协议栈比平台自己的 `fetch` 只贵约 14%** —— 十亿请求下 $367 对 $321 —— 换来的是
平台的 `fetch` 根本做不到的事。

冷启动没有进表，因为它占多大比重取决于你的流量有多常落在全新 isolate 上。实测每个 isolate 约服务 17.8 个请求，
摊下来爬升段每请求约 4.4 ms —— 十亿请求下约合每月 **$87**。`warmup()` 能消掉其中大部分：

| | 摊薄后的爬升 | 十亿请求/月 | `warmup()` 自身成本 | 净 |
| --- | --- | --- | --- | --- |
| 不调用 | 4.4 ms/请求 | $87 | — | — |
| `warmup()` | 2.8 ms/请求 | $56 | 10 ms 启动 | **省 $31** |
| `warmup({ iterations: 5 })` | 1.1 ms/请求 | $22 | 22 ms 启动 | **省 $65** |

表里 `warmup()` 自身成本记为零，是因为 Workers Standard 不对启动 CPU 计费。在会计费的形态上，那 22 ms 每个
isolate 只收一次、并摊给该 isolate 服务的请求——十亿请求、每 isolate 17.8 个请求的话是每月 $25，对着省下的 $65，
**仍然是净赚**。低于约每 isolate 7 个请求，它就不再回本。
