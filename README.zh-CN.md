...
**这些数字替换掉了一批测错的,而错在哪里值得写下来。** 它们来自的那个源站重复的是一段 150 字节的 HTML 片段,gzip 把它压到 **220:1**——所以一个"1 MB 正文"在线上只有四千字节,针对它做的每一次测量都只量到了解压,而把 TLS 记录和流处理的**每线上字节**成本整个抹掉了。真实页面大约 4:1。现在源站重复的是 256 KiB 真实压缩后的 JavaScript,压缩比 2.8:1——gzip 的窗口是 32 KiB,这么大的重复周期压不掉。

**修正幅度很大。** 正文偏重的那几行是 1.4.0 及以前所写数字的**两到三倍**,而且再怎么讲究中位数还是最小值都发现不了——因为那些数字**自洽**,它们只是在回答错误的问题。另一次独立测量印证了这一点:从 CDN 抓一个真实的 3.6 MB 文件是 142 ms,这里预测 120 ms。

读这张表要留意两点。2.8:1 的内容比典型页面**略难压**,所以这些值偏保守而不是偏乐观;以及这个平台上 CPU 在不同 isolate 之间有最多约 1.5 倍波动,所以形状比任何单个数字更要紧。
